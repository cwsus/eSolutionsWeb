...
<body>
<header class="wpthemeHeader" role="banner" tabindex="-1">
	<div>
...
<c:choose><c:when test="<%=currentNavLevel > previousNavLevel%>"><% for(int i=currentNavLevel; i > previousNavLevel; i--){%><ul <% if(previousNavLevel==0){%> role="navigation" tabindex="-1" class="wpthemeSideNav"<%}%> ><%} %></c:when><c:when test="<%=currentNavLevel < previousNavLevel %>"><% for(int i=currentNavLevel; i < previousNavLevel; i++){%></ul></li><%} %></c:when></c:choose><li <% if (isNodeSelected) { %>id="portalSelectedNode" <% } %> ><span class="<%=rowCssClass%>">
...
<%-- MAIN CONTENT --%>
	<div class="wpthemeMainContent" role="main" tabindex="-1">
