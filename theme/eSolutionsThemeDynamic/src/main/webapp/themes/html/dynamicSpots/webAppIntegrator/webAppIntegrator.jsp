...
<c:set var="isDebug" value="${wp.themeConfig['wai.isDebug']}" scope="page"/>
<c:set var="dirMD" value="${wp.selectionModel.selected.metadata}" scope="page"/>
<c:set var="currentNavNodeID" scope="request" value="${wp.identification[wp.selectionModel.selected]}"/>

<%--get theme template--%>
<c:choose>
<c:when test="${!empty pageScope.dirMD['com.ibm.portal.theme.template.file.name.html']}">
    <c:set var="waiCustomTemplate" value="${pageScope.dirMD['com.ibm.portal.theme.template.file.name.html']}" scope="page"/> <%--use theme template set for the page--%>
    <c:if test="${pageScope.waiCustomTemplate == 'ic4_wai'}">
        var ic4_wai_integration = true;
        <c:set var="waiCustomTemplate" value="" scope="page"/>
    </c:if>
</c:when>
<c:otherwise>
    <c:set var="waiCustomTemplate" value="" scope="page"/> <%--use default--%>
</c:otherwise>
</c:choose>

<c:set var="markupPath" value="${wp.themeList.current.contextRoot}/themes/html/${wp.themeList.current.resourceRoot}/webAppIntegrator/markup${pageScope.waiCustomTemplate}" scope="request"/>
...
if (typeof waiScriptInstance == "undefined") {
    ...
    if ("<c:out value="${pageScope.waiCustomTemplate}"/>"=="SideNav"){
...
%>
        ...
        <r:dataSource uri='<%="spa:" + pageContext.getAttribute("currentNavNodeID", PageContext.REQUEST_SCOPE)%>' escape="none" var="<%=OUTPUT_VAR%>">
            <r:param name="themeURI" value="res:${requestScope.markupPath}/${markupJsp}"/>
...
<%
    ...
    if (portalContext != null && portalContext != "") {
