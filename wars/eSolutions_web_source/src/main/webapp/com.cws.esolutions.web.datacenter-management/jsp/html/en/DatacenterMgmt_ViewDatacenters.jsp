    ...
    <div class="wrapper">
        ...
        <ul id="latestnews">
            <c:forEach var="datacenter" items="${datacenterList}">
                <li><a href="<c:url value='/ui/datacenter-management/datacenter/${datacenter.guid}' />" title="${datacenter.name}">${datacenter.name}</a></li>
            </c:forEach>
        </ul>
                ...
                <tr>
                    <c:forEach begin="1" end="${pages}" var="i">
                        <c:choose>
                            ...
                            <c:otherwise>
                                <td>
                                    <a href="<c:url value='/ui/datacenter-management/list-datacenters/page/${i}' />" title="${i}">${i}</a>
                                </td>
                            </c:otherwise>
                        </c:choose>
                    </c:forEach>
