...
<div id="content">
    ...
    <c:choose>
        <c:when test="${not empty articleData}">
          ...
          <input type="button" name="execute" value="<spring:message code='knowledgebase.approve.article' bundle='${theme}' />" id="execute" class="submit" onclick="disableButton(this); validateForm(this.form);" />
          ...
          <input type="button" name="cancel" value="<fmt:message key='theme.button.cancel.text' bundle='${theme}' />" id="cancel" class="submit" onclick="redirectOnCancel('${pageContext.request.contextPath}/ui/knowledge-management/list-pending');" />
            </form:form>
        </c:when>
        <c:otherwise>
